#!/bin/bash
# Port a single great-tables blog post
# Usage: ./scripts/port-great-tables-post.sh <folder-name>
# Example:
...
SOURCE="_external-sources/great-tables/docs/blog/$FOLDER"
DEST="content/blog/great-tables/$FOLDER"
  ...
  awk '
    ...
    /^---$/ {
      if (in_fm == 0) {
        ...
        if (!added_porting) {
          print "ported_from: great-tables"
...
# 3. Render with Quarto from content/blog/great-tables/ (using uv environment)
echo "  Rendering with Quarto..."
(cd content/blog/great-tables && uv run quarto render "$FOLDER/index.qmd" --to hugo-md 2>/dev/null) || {
