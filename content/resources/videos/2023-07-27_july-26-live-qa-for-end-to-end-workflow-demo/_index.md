---
image: thumbnail.jpg
date: '2023-07-27'
description: 'Live Q&A Session for the End-to-End Workflow Demo on July 26th.


  If you ended up here first, the demo is happening here: https://youtu.be/1uxAHbLh_rM


  Ep 4. Standardizing access & ensuring consistent data in data products (using FastAPI & Posit Team) | End-to-end workflows


  Follow-up links:

  * Demo recording: https://youtu.be/1uxAHbLh_rM

  * Posit Team: https://posit.co/products/enterprise/team/

  * Talk to us directly: https://posit.co/schedule-a-call/?booking_calendar__c=RST_YT_Demo

  * Follow-along blog post: To be added

  * Source code for example: https://github.com/ryjohnson09/fastAPI_demo

  * Posit Team demo resources: pos.it/demo-resources'
people: []
resource_type: video
resources: []
software: []
title: July 26 | Live Q&A for End-to-End Workflow Demo

external:  # updated automatically, do not edit
  channel: Posit PBC
  comment_count: 0
  date: '2023-07-27T03:48:43Z'
  definition: hd
  description: 'Live Q&A Session for the End-to-End Workflow Demo on July 26th.


    If you ended up here first, the demo is happening here: https://youtu.be/1uxAHbLh_rM


    Ep 4. Standardizing access & ensuring consistent data in data products (using FastAPI & Posit Team) | End-to-end workflows


    Follow-up links:

    * Demo recording: https://youtu.be/1uxAHbLh_rM

    * Posit Team: https://posit.co/products/enterprise/team/

    * Talk to us directly: https://posit.co/schedule-a-call/?booking_calendar__c=RST_YT_Demo

    * Follow-along blog post: To be added

    * Source code for example: https://github.com/ryjohnson09/fastAPI_demo

    * Posit Team demo resources: pos.it/demo-resources'
  duration: 1262
  has_captions: false
  language: en
  last_updated: '2026-03-04T14:51:25.546030+00:00'
  like_count: 9
  playlist: ''
  tags: []
  thumbnail: https://i.ytimg.com/vi/nRGWO_9OovI/maxresdefault.jpg
  title: July 26 | Live Q&A for End-to-End Workflow Demo
  url: https://www.youtube.com/watch?v=nRGWO_9OovI
  view_count: 417
---
image: thumbnail.jpg

