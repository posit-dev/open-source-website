---
title: "Posit Conf 2025"
layout: single
---

## Production

{{< insert-items cols=3 >}}
- videos/2025-11-07_building-multilingual-data-science-teams-michael-thomas-ketchbrook-analytics-positconf2025
- videos/2025-11-07_building-multilingual-data-science-teams-michael-thomas-ketchbrook-analytics-positconf2025
- videos/2025-11-07_building-multilingual-data-science-teams-michael-thomas-ketchbrook-analytics-positconf2025
- videos/2025-11-07_r-python-playing-nice-in-production-claudia-penaloza-continental-tires-positconf2025
- videos/2025-11-07_purrrfectly-parallel-purrrfectly-distributed-charlie-gao-posit-positconf2025
- videos/2025-11-07_data-science-hangout-rachael-dempsey-posit-amp-matthew-montero-genre-positconf2025
{{< /insert-items >}}


### Blog Post

{{< insert-item item="blog/anscombes-quartet" hide-badge=true >}}

### 3 Columns

{{< insert-items cols=3 >}}
- blog/anscombes-quartet
- people/jeroen-janssens
- blog/air
{{< /insert-items >}}

## Grids

### 2 Columns
{{< insert-items cols=2 >}}
- people/greg-swinehart
- software/ggplot2
- events/pydata-boston-2025
- blog/air
{{< /insert-items >}}

### 3 Columns
{{< insert-items cols=3 >}}
- software/ggplot2
- software/shiny-r
- software/dplyr
- cheatsheets/data-visualization
- cheatsheets/data-transformation
- videos/2026-03-04_charlie-gao-advances-in-the-shiny-ecosystem
{{< /insert-items >}}

### 4 Columns
{{< insert-items cols=4 >}}
- people/hadley-wickham
- people/carson-sievert
- people/barret-schloerke
- people/carlos-scheidegger
- software/ggplot2
- software/shiny-r
- software/dplyr
- software/quarto
{{< /insert-items >}}

### 3 Columns (no badges)
{{< insert-items cols=3 hide-badge=true >}}
- people/hadley-wickham
- software/ggplot2
- blog/air
{{< /insert-items >}}

### 5 Columns
{{< insert-items cols=5 >}}
- software/ggplot2
- software/shiny-r
- software/dplyr
- software/air
- software/quarto
- software/gt
- software/pointblank
- software/vetiver-r
- software/pins-r
- software/htmltools
{{< /insert-items >}}

### 6 Columns
{{< insert-items cols=6 >}}
- software/ggplot2
- software/shiny-r
- software/dplyr
- software/air
- software/quarto
- software/gt
- software/pointblank
- software/vetiver-r
- software/pins-r
- software/htmltools
{{< /insert-items >}}

## Single Cards

### Person
{{< insert-item item="people/hadley-wickham" >}}

### Software
{{< insert-item item="software/ggplot2" >}}

### Event
{{< insert-item item="events/pydata-boston-2025" >}}


### Resource (Cheatsheet — shorthand)
{{< insert-item item="cheatsheets/data-visualization" >}}

### Resource (Video — shorthand)
{{< insert-item item="videos/2026-03-04_charlie-gao-advances-in-the-shiny-ecosystem" >}}

### Software (no badge)
{{< insert-item item="software/ggplot2" hide-badge=true >}}

### Event (no badge)
{{< insert-item item="events/pydata-boston-2025" hide-badge=true >}}

## Tile Format

### Tiles (mixed content types)
{{< insert-items format="tile" >}}
- software/ggplot2
- blog/anscombes-quartet
- people/hadley-wickham
- events/pydata-boston-2025
- videos/2026-03-04_charlie-gao-advances-in-the-shiny-ecosystem
- cheatsheets/data-visualization
{{< /insert-items >}}

### Single Tile
{{< insert-item item="software/ggplot2" format="tile" >}}

## Row Format

### Row (software)
{{< query-items path="/software/.*" sort-by="title" sort-direction="asc" limit=8 format="row" >}}

### Row (insert-items)
{{< insert-items format="row" >}}
- blog/anscombes-quartet
- people/hadley-wickham
- software/ggplot2
- events/pydata-boston-2025
{{< /insert-items >}}

### Row (custom height h-20)
{{< query-items path="/software/.*" sort-by="title" sort-direction="asc" limit=4 format="row" height="h-20" >}}

### Tile (custom height h-24)
{{< insert-items format="tile" height="h-24" >}}
- software/ggplot2
- blog/anscombes-quartet
- people/hadley-wickham
{{< /insert-items >}}

