...
![Meme from r/ProgrammerHumor](img/1.png){fig-alt="Comic: on Friday evening a programmer says 'I'll finish this on Monday'; on Monday morning they stare at their code asking 'What does this mean?!'"}
...
![Project structure](img/2.png){fig-alt="File explorer showing the project directory tree: folders for _extensions, .quarto, code, data_processed, data_raw, extras, manuscript, outputs, renv, and slides, plus config files like _quarto.yml, .gitattributes, .gitignore, .Rprofile, README.md, and renv.lock"}
...
![Collapsible sections in Positron](img/3-pos.png){fig-alt="Positron IDE showing collapsible code sections created with the '# Section ----' syntax"}

![Collapsible sections in RStudio](img/3-rstudio.png){fig-alt="RStudio IDE showing collapsible code sections created with the '# Section ----' syntax"}
...
![Default Quarto PDF template](img/4.png){fig-alt="PDF output from the default Quarto template showing title, author, and abstract"}
...
![Custom Quarto PDF template](img/5.png){fig-alt="PDF output from the custom orcid extension template with structured affiliations, ORCID identifiers, and keywords"}
...
![Quarto Beamer template](img/6.png){fig-alt="Beamer presentation slides generated from Quarto showing a title slide and a content slide with a table"}
