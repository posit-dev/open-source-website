...
image: featured.jpg
image-alt: >
  A vivid green and pink aurora arcing across a star-filled night sky over
...
The core concept in OpenTelemetry is a **trace**: the full path of a request through your system. Each trace is made up of **spans** -- individual units of work with a name and a duration. Spans nest inside each other, so you can follow how each operation triggered the next:
...
To make this concrete, let's look at a Shiny chat app built with [shinychat](https://posit-dev.github.io/shinychat/) and [ellmer](https://ellmer.tidyverse.org) that fetches weather forecasts. It uses mirai for async execution and httr2 for weather API requests.
...
During development, you can also run an [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) on your own machine and inspect traces directly, without sending data anywhere else.
...
If you're deploying content to Posit Connect, refer to [how to set environment variables on Posit Connect](https://docs.posit.co/connect/user/content-settings/#setting-env-vars).
