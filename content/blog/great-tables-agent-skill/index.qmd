...
thing. [**Great Tables**][great-tables] already has everything you need to
...
what an [**Agent Skill**][agent-skills] is for: a folder of instructions the
...
![](comparator-score.png){width="95%" fig-align="center" fig-alt="Box plot of the skill's comparator score across three repeats per prompt, with per-prompt means ranging from about 71 percent on the hardest prompts to 94 percent on the easiest, against a baseline of about 3 to 26 percent with no skill loaded."}
...
![](consistency.png){width="95%" fig-align="center" fig-alt="Range plot of the skill's minimum, mean, and maximum comparator score across three repeats per prompt, with baseline scores plotted alongside for reference, showing a repeat-to-repeat spread from under 1 percentage point on the tightest prompt to about 52 points on the loosest."}
...
![](skill-table.png){width="82%" fig-align="center" fig-alt="The skill's GT Cars table again, shown for direct comparison against the ground truth above."}
...
project][gt-skill], along with the harness that ran this evaluation and the
