...
```{python}
agent = (  # <1>
    pb.Validate(sales_data)  # <1>
    .col_vals_not_null(columns="date")  # <1>
    .col_vals_between(columns="sales", left=0, right=10000)  # <1>
    .col_vals_between(columns="customer_rating", left=1.0, right=5.0)  # <1>
    .col_vals_in_set(columns="region", set=["North", "South", "East", "West"])  # <1>
    .interrogate()  # <1>
)  # <1>
...
```{python}
sales_data.llm.use("ollama", "llama3.2")  # <1>

sales_with_performance = sales_data.llm.classify(  # <2>
    "sales",  # <2>
    ["high", "medium", "low"],  # <2>
    pred_name="performance",  # <2>
)  # <2>

sales_with_performance.select(  # <3>
    ["date", "region", "product", "sales", "performance"]  # <3>
)  # <3>
```

1. Configure the LLM connection (using Ollama with a local model)
2. Classify sales performance based on the sales amount
3. View the results

Or we can generate custom descriptions for each product:

```{python}
sales_with_description = sales_data.llm.custom(  # <1>
    "product",  # <1>
    pred_name="description",  # <1>
    prompt="Create a brief, compelling marketing description for this product in 10 words or less",  # <1>
)  # <1>

sales_with_description.select(["product", "description"])  # <2>
```

1. Generate a marketing description based on the product name
2. View the results
