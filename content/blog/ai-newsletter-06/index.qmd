...
image-alt: "A horizontal bar chart, shown as a white card on a mauve gradient, titled 'Frontier models still struggle to notice subtle data quality issues.' Bars show benchmark scores by model, colored by lab: Gemini 3.5 Flash and Claude Fable 5 lead in the mid teens, while OpenAI's GPT models all score under 10%."
topics:
...
# Eval plots use the default ggplot theme (they mimic real analysis output);
# plots presenting bluffbench2's results use this branded theme.
theme_bluff <-
...
#| fig-align: center
labs <- read.csv(
  "https://raw.githubusercontent.com/posit-dev/bluffbench2/main/inst/csv/labs.csv"
)
...
smooth_summary %>%
  ...
  theme(
    legend.location = "plot",
    legend.position = "inside",
    legend.position.inside = c(.78, .28),
    plot.title.position = "plot",
    plot.subtitle = ggtext::element_markdown()
  )
```
...
- [**Introducing bluffbench**](https://posit.co/blog/introducing-bluffbench): Writeup of the original eval.
...
- [**_It's (still) very bad to be wrong_**](https://skaltman.github.io/scipy-2026/): Slides from our recent SciPy 2026 talk on building agents for correct, transparent, and reproducible data analysis in light of the bluffbench results. 
