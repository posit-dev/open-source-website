...
This list is roughly ordered from most straightforward to most complicated.[^1]
...
[^1]: One interface we didn't mention here is 'custom agents,' a concept popularized by GitHub Copilot and now appearing under various names. These bundle some combination of prompts and tools (sometimes gathered via MCP). We'd reach for the options mentioned above first, which are open standards that are broadly supported across most agent platforms and thus can be shared and migrated more easily.
