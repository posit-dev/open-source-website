...
image-alt: >-
  ...
  with no skill loaded, a plain title with no subtitle, no color, and no
  source note; on the right, generated with the house skill loaded, showing
  a navy header band, a blue sequential heatmap on price, a shaded stub
  column, and alternating row stripes.
topics:
...
Tables**][great-tables] already has everything needed to build a genuinely
...
Skill**][agent-skills] is supposed to do, but a skill's own design is itself a
...
![](comparator-score.png){width="95%" fig-align="center" fig-alt="Box plot of the house skill's comparator score across three repeats per prompt, ranging from about 54 percent on the hardest prompt to 97 percent on the easiest, against a baseline of 9 to 20 percent with no skill loaded."}
...
![](consistency.png){width="95%" fig-align="center" fig-alt="Range plot of the house skill's minimum, mean, and maximum comparator score across three repeats per prompt, with baseline scores plotted alongside for reference, showing a repeat-to-repeat spread from about 1 to 55 percentage points depending on the prompt."}
...
![](token-usage.png){width="95%" fig-align="center" fig-alt="Scatter plot of total tokens per invocation for the house skill against a no-skill baseline, by prompt, with skill runs generally between 50,000 and 170,000 tokens against baseline runs mostly under 40,000."}
...
design, in [Hrudith Lakshminarasimman's `gt-skill` project][gt-skill] — the
...
[agent-skills]: https://posit-dev.github.io/great-docs/skills.html
[gt-skill]: https://github.com/HrudithL/gt-skill
