...
people:
  ...
  - Teun Van den Brand
description: >
  ggsql 0.4.1 adds support for spatial plotting and data aggregation within
...
ggsql continues to evolve and today we are pleased to announce the release of ggsql 0.4.1. This release is heavy on features and we hope this blog post can get you excited about where we are taking the project.

There are two headline features for 0.4.1: Spatial plotting and within-layer aggregation of data. Apart from these two there is a grab back of smaller improvements we will list in the end.
...
The `geom` column in the dataset contains the country border data in a binary format called [Well-Known Binary](https://libgeos.org/specifications/wkb/) (WKB) which will be decoded by ggsql into the actual coordinates. In general, as long as the data in the geometry column can be converted to WKB by the backend it can be used.
...
### ADBC reader
The reception to our alpha release was amazing and users have been quick to request support for their backend of choice. We can't wait to support as many databases as possible but we have held off for a bit on most to get other infrastructure in place. 

One exception we make in this release is the addition of a general ADBC reader. We have decided to include this now since it opens the door for a huge swath of backends right out of the gate. Together with the ODBC reader there should now be rudimentary support for a great deal of databases as many have drivers for either ODBC or ADBC.
