...
In the [1.1.0 post on vctrs](/blog/2023-02-02_dplyr-1-1-0-vctrs/), we discussed that we've rewritten all of dplyr's vector functions on top of [vctrs](https://vctrs.r-lib.org/) for improved versatility.
...
In the [1.1.0 post on joins](/blog/2023-01-31_dplyr-1-1-0-joins/), we discussed the new `multiple` argument that was added to `left_join()` and friends, which had a built in safety check that warned when you performed a join where a row from `x` matched more than one row from `y`.
