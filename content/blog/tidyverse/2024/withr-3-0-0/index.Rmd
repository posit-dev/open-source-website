...
Until now, we haven't been able to leverage these contributions because of our policy of [supporting the current and previous four versions of R](/blog/2019-04-01_r-version-support/). Now that enough time has passed, it was time for a rewrite! Our version of `base::on.exit()` is `withr::defer()`. Along with better default behaviour, `withr::defer()` allows the clean up of resources non-locally (ironically an essential feature for implementing `local_` functions). Given the changes in R 3.5.0, `withr::defer()` can now be implemented as a simple wrapper around `on.exit()`.
