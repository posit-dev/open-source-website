...
The ggplot2 package has had a [recent release](/blog/2025-09-11_ggplot2-4-0-0/) that included some relevant changes to styling plots.
...
Let's say you want to find out exactly which elements have multiple parents.
...
At the time of writing, it has `r length(formals(theme))` arguments and `...` is obfuscating additional options.
...
A trick you can pull to have legends eat up all the available real estate, is to give them `"null"`-unit size.
Below, that trick stretches the colourbar across the full width of the plot.

```{r stretchy_colourbar}
p +
...
Setting the typography of your plots is important and discussed more thoroughly in [this blog post](/blog/2025-05-12_fonts-in-r/). 
