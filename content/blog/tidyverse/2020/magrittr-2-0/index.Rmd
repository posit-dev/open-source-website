...
__Update 2020-09-22:__ Some minor issues have arised from the first round of beta testing (see note on visibility and laziness below). We postponed the release to late October to give more time for potential further issues to surface before publication of magrittr 2.0 to CRAN.
...
__Update 2020-09-22:__ Another issue caused by laziness is that if any function in a pipeline returns invisibly, than the whole pipeline returns invisibly as well.
...
__Update 2020-09-22:__ The issue you're most likely to encounter is that using `return()` inside `{` inside `%>%` is no longer supported. If you do this, you will see this error:
