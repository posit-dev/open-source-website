...
I'm super excited to announce that [querychat](https://posit-dev.github.io/querychat/) now supports [ggsql](../../blog/2026-04-20_ggsql_alpha_release/) (a grammar of graphics for SQL). This means that users of querychat can now go beyond predetermined dashboard views and ask the LLM for bespoke visualizations on the fly. With this addition, `querychat` now has three fundamental tools at its disposal for natural language data exploration:

1.  **Visualize:** The LLM generates `ggsql` queries to create visualizations, rendered inline in the chat.
2.  **Query:** The LLM generates SQL queries to compute numerical summaries, rendered as text or tables in the chat.
3.  **Filter:** The LLM generates filter SQL queries to update a reactive data frame (`df()`) that can be used to drive other outputs in an app, like plots, tables, value boxes, etc.

Crucially, every one of these tools works by executing SQL --- never arbitrary code. That's an intentional design choice: SQL execution can be locked down so that the LLM literally cannot modify your data, making it viable for production environments where security and control aren't optional. Every query is also fully visible and reproducible: users can inspect the SQL behind any response, and copy it to run independently.

Let's see what this looks like in practice. For more, see [querychat's website](https://posit-dev.github.io/querychat/).

## Quick demo

The video below shows `querychat` in action --- querying, filtering, and visualizing data through natural language. The demo app also uses [Snowflake Semantic Models](https://www.snowflake.com/en/developers/guides/snowflake-semantic-view/) to give the LLM access to authoritative business logic and metric definitions. [Source code](https://github.com/sol-eng/snowflake_demo/tree/main/apps/shiny-python-querychat) is available for reference, though you'll need your own Snowflake account and credentials to run it.

TODO: A minimal video of a real querychat app experience.
