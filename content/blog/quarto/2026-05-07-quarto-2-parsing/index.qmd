...
Accepting the reality that we all make mistakes allows us to raise our expectations about what computers should do for us. In this context, we should want computers to provide users with good guidance about sources of errors and how to fix them.
This requirement provided the initial motivation for a formal grammar of the Quarto Markdown ("qmd") dialect using the [Tree-sitter system](https://tree-sitter.github.io/tree-sitter/).
Because we have an actual grammar, documents that fail to parse as Markdown must be fixed before output is produced. 
But this trade-off allows us to provide contextual feedback in editors and in the command-line tooling.
...
In our view, this means that syntax changes become _less_ enticing over time; as a result, in Quarto 2, we've replaced the front end of our pipeline with a dedicated parser and AST processor,
