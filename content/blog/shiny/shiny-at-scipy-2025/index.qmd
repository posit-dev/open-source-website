---
title: Posit at SciPy 2025
subtitle: Tacoma, WA (July 2025)
people:
  - Shiny Team
date: "2025-07-14"

image-video: scipy-og.mp4
image: scipy-og.jpg
image-alt: "Shiny at SciPy 2025"

listing:
  - id: talks
    template: talks.ejs
    contents:
      - talks.yml
ported_from: shiny
port_status: in-progress
software: ["shiny-python"]
languages: ["Python"]
categories:
  - Interactive Apps
---

::: {lead}
**The Shiny Team was at SciPy 2025!**
If you missed us at [SciPy 2025](https://www.scipy2025.scipy.org/) here's what we were up to.
:::

![posit shiny team at our scipy booth](scipy-posit-shiny-team.jpg)

We just got back from a week at [SciPy 2025](https://www.scipy2025.scipy.org/) and had a great time meeting and talking with folks at the conference.
If you didn't get a chance to say hi to us,
you can also find us online on [<i class="bi bi-github"></i> GitHub](https://github.com/posit-dev/py-shiny) or
[<i class="bi bi-discord"></i> Discord](https://discord.com/invite/yMGCamUMnS).
You can also learn more about Posit at [<i class="bi bi-house-fill"></i> posit.co](https://posit.co).

If you missed a tutorial or talk we have all the information below.
You'll have to wait a bit for the SciPy folks to get all the tutorials and talks uploaded to [<i class="bi bi-youtube"></i> Youtube](https://www.youtube.com/@SciPy-Conf/videos).

## Talks by Posit Folks

::: {#talks}
:::
