...
```{r pca-tidy, warning = FALSE, fig.height=4.25, dev = 'svg', dev.args = list(bg = "transparent")}
library(tidymodels)
...
```{r pls, fig.height = 3, fig.align="center", fig.height=4, dev = 'svg', dev.args = list(bg = "transparent")}
pls_rec <- 
  recipe(water + fat + protein ~ ., data = meat_tr) %>% 
  step_normalize(all_predictors()) %>% 
  step_pls(
...
```{r pls-coef, fig.align="center", dev = 'svg', dev.args = list(bg = "transparent")}
tidy(pls_rec, id = "pls") %>%
