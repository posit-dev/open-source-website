...
image: "featured.png"
image-alt: "The package hex sticker, showing a monkey hanging from a wooden bridge with a jungle background. Behind the hex sticker is a subtle yellow-to-green gradient."
software:
 - mcptools
topics:
...
cat(sub(
  "![](",
  "![A brown and white Border Collie on a deck, looking attentively at the camera, with wire railings and blurred greenery in the background.](",
  contents_markdown(tools$fetch_reference_image()),
  fixed = TRUE
))
```
...
```{r}
#| echo: false
#| eval: !expr params$redeploy
app_dir <- tempfile()
dir.create(app_dir)

writeLines(
  c("engine: mcptools", "tools: tools.R"),
...
writeLines(
  ...
  file.path(app_dir, "tools.R")
)

rsconnect::deployAPI(app_dir, contentCategory = "mcp", forceUpdate = TRUE)

deployed_url <- rsconnect::deployments(app_dir)$url
```
...
config <- list(
  mcpServers = list(
    connect = list(
      url = if (exists("deployed_url")) deployed_url else Sys.getenv("MCPTOOLS_CONNECT_DEMO"),
      headers = list(Authorization = "Key ${CONNECT_API_KEY}")
    )
  )
)

jsonlite::write_json(config, "config.json", auto_unbox = TRUE, pretty = TRUE)
withr::defer(unlink("config.json"))
```
