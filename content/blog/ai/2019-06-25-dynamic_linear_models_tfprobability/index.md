...
Our example is taken from Petris et al.(2009)(Petris et al. 2009), chapter 3.2.7. [^2]
...
<figure>
<img src="images/capm_overview.png" width="500" alt="Monthly returns for selected stocks; data from Berndt (1991)." />
<figcaption aria-hidden="true">Monthly returns for selected stocks; data from Berndt (1991).</figcaption>
</figure>

The [Capital Asset Pricing Model](https://en.wikipedia.org/wiki/Capital_asset_pricing_model) then assumes a linear relationship between the excess returns of an asset under study and the excess returns of the market. For both, *excess returns* are obtained by subtracting the returns of the chosen *risk-free* asset; then, the scaling coefficient between them reveals the asset to either be an "aggressive" investment (slope \> 1: changes in the market are amplified), or a conservative one (slope \< 1: changes are damped).
...
<figure>
<img src="images/capm_forecast.png" width="500" alt="12-point-ahead forecasts for IBM; posterior means +/- 2 standard deviations." />
<figcaption aria-hidden="true">12-point-ahead forecasts for IBM; posterior means +/- 2 standard deviations.</figcaption>
</figure>
...
<figure>
<img src="images/capm_smoothed.png" width="500" alt="Smoothing estimates from the Kálmán filter. Green: coefficient for dependence on excess market returns (slope), orange: vector of ones (intercept)." />
<figcaption aria-hidden="true">Smoothing estimates from the Kálmán filter. Green: coefficient for dependence on excess market returns (slope), orange: vector of ones (intercept).</figcaption>
</figure>
...
<figure>
<img src="images/capm_filtered.png" width="500" alt="Filtering estimates from the Kálmán filter. Green: coefficient for dependence on excess market returns (slope), orange: vector of ones (intercept)." />
<figcaption aria-hidden="true">Filtering estimates from the Kálmán filter. Green: coefficient for dependence on excess market returns (slope), orange: vector of ones (intercept).</figcaption>
</figure>
...
<figure>
<img src="images/airp.png" width="500" alt="AirPassengers." />
<figcaption aria-hidden="true">AirPassengers.</figcaption>
</figure>
...
The model is a *sum* -- cf. [sts_sum](https://rstudio.github.io/tfprobability/reference/sts_sum.html) -- of a linear trend and a seasonal component:
...
<figure>
<img src="images/airp_forecast.png" width="500" alt="AirPassengers, 12-months-ahead forecast." />
<figcaption aria-hidden="true">AirPassengers, 12-months-ahead forecast.</figcaption>
</figure>
...
<figure>
<img src="images/airp_components.png" width="500" alt="AirPassengers, decomposition into a linear trend and a seasonal component (both centered)." />
<figcaption aria-hidden="true">AirPassengers, decomposition into a linear trend and a seasonal component (both centered).</figcaption>
</figure>
...
[^1]: For an excellent yet concise introduction to state space models, see chapter 18 of Murphy (2012) (Murphy 2012).

[^2]: A draft version is available online at http://people.bordeaux.inria.fr/pierre.delmoral/dynamics-linear-models.petris_et_al.pdf. However, the draft does not yet fully elaborate on this example.

[^3]: Forecasting functionality is not provided by `dlm` for dynamic linear models, see `?dlmForecast`

[^4]: We do not think it instructive to display the results obtained by Petris et al. here, as this post does not aim to evaluate or compare implementations.

[^5]: It is still clipped up and below though. We've been trying to find a compromise between local visibility of the filtered means on the one hand, and global visibility of uncertainty in the estimates on the other.

[^6]: For MCMC, you can straightforwardly adapt the above example.
