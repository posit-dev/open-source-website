...
Once you've installed and loaded `torchvisionlib`, you have the choice among an impressive number of [image recognition-related models](https://pytorch.org/vision/stable/models.html). The process, then, is two-fold:
...
Well, maybe; but maybe not. The far more sustainable solution is to make it reasonably easy to extend `torch` in small, dedicated packages that each serve a clear-cut purpose, and are fast to install. A detailed and practical walkthrough of the process is provided by the package [`lltm`](https://github.com/mlverse/lltm). This package has a recursive touch to it. At the same time, it is an instance of a C++ `torch` extension, *and* serves as a tutorial showing how to create such an extension.
