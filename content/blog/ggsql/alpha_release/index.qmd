...
```{ggsql}
WITH astronauts AS (                            -- <1>
  SELECT * FROM 'astronauts.parquet'            -- <1>
  QUALIFY ROW_NUMBER() OVER (                   -- <1>
    PARTITION BY name                           -- <1>
    ORDER BY mission_number DESC                -- <1>
  ) = 1                                         -- <1>
)                                               -- <1>

VISUALIZE age AS x, category AS fill            -- <2>
DRAW histogram                                  -- <3>
  SETTING binwidth => 1, position => 'identity' -- <3>
PLACE rule                                      -- <4>
  SETTING x => [34, 44], linetype => 'dotted'   -- <4>
PLACE text                                      -- <5>
  SETTING                                       -- <5>
    x => [34, 44, 60],                          -- <5>
    y => [66, 49, 20],                          -- <5>
    label => [                                  -- <5>
      'Mean age at selection = 34',             -- <5>
      'Mean age at mission = 44',               -- <5>
      'John Glenn was 77\non his last mission -\nthe oldest person to\ntravel in space!' -- <5>
    ],                                          -- <5>
    hjust => 'left',                            -- <5>
    vjust => 'top',                             -- <5>
    offset => [10, 0]                           -- <5>
SCALE fill TO accent                            -- <6>
LABEL                                           -- <7>
  title => 'How old are astronauts on their most recent mission?', -- <7>
  subtitle => 'Age of astronauts when they were selected and when they were sent on their mission', -- <7>
  x => 'Age of astronaut (years)',              -- <7>
  fill => null                                  -- <7>
```
