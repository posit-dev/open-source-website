...
2.  Plot composition. One plot is good, many plots are (sometimes) better. Inspired by what the R patchwork package has done for ggplot2 plots we want to allow composition of plots, tables, and perhaps more. Who knows if one day a single SQL query can create a beautiful dashboard. The good news is that the new renderer has all of the composition logic built in already (faceting is done through that API), so much of this is a question of landing the right syntax (something we take extremely seriously)
...
These are the main focus for us, but the writer switch has itself offered so much low-hanging fruit for us to pick, that there will be plenty of user visible additions over the next year.
