...
- *Italic* (`*`), **Bold** (`**`), [Underline]{.underline} (`_`), [Strikethrough]{style="text-decorationline-through"} (`~~`), <sub>Subset</sub> (`~`), <sup>Superset</sup> (`^`), `code` (`` ` ``), [Links](#rich-text-support-through-extended-markdown) (`[text](url)`)
- Headings 1 — 6 (`#` — `######`)
- Quote blocks (`>`)
- Code blocks (```` ``` ````)
